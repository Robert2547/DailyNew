...
class Settings(BaseSettings):
    ...
    POSTGRES_HOST: str = os.getenv("POSTGRES_HOST", "auth_db")
    ...
    def get_database_url(self) -> str:
        ...
        url = f"postgresql://{self.DB_USER}:{self.DB_PASSWORD}@{self.POSTGRES_HOST}:{self.AUTH_DB_PORT}/{self.AUTH_DB_NAME}"
        logger.info(f"Using production database URL: {url}")
        return url
